-- ============================================================
-- Flyway Migration: V2__rule_handler_refactor.sql
-- Description: 规则-处理器系统重构数据库变更
-- Author: Claude Code
-- Date: 2026-02-11
-- ============================================================
...
-- Note: priority column already exists in the current schema
-- Add handler_config and on_error_policy columns
ALTER TABLE `message_rule`
ADD COLUMN `handler_config` JSON DEFAULT NULL COMMENT 'Handler 配置（JSON 格式）' AFTER `response_template`,
ADD COLUMN `on_error_policy` VARCHAR(20) NOT NULL DEFAULT 'STOP' COMMENT '错误策略: STOP, CONTINUE, LOG_ONLY' AFTER `handler_config`;

-- Add additional columns for compatibility
ALTER TABLE `message_rule`
ADD COLUMN `deleted` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '逻辑删除: 0=未删除, 1=已删除' AFTER `enabled`,
ADD COLUMN `create_by` VARCHAR(50) DEFAULT NULL COMMENT '创建人' AFTER `deleted`,
ADD COLUMN `update_by` VARCHAR(50) DEFAULT NULL COMMENT '更新人' AFTER `create_by`;

-- Note: Column renames are skipped to avoid foreign key issues
-- We'll use aliases in the entity mapping instead
-- The entity will map: name -> ruleName, created_at -> createTime, updated_at -> updateTime

-- Ensure priority index exists (may already exist)
CREATE INDEX IF NOT EXISTS `idx_priority` ON `message_rule` (`priority`, `id`);
...
CREATE TABLE IF NOT EXISTS `rule_policy` (
  ...
  `rule_id` BIGINT UNSIGNED NOT NULL COMMENT '规则 ID（外键）',
...
-- ============================================================
-- 3. 创建 handler_chain 表
-- ============================================================

CREATE TABLE IF NOT EXISTS `handler_chain` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Handler 链 ID',
  `rule_id` BIGINT UNSIGNED NOT NULL COMMENT '规则 ID（外键）',
...
-- ============================================================
-- 6. 数据迁移：为现有规则创建默认 handler_chain（兼容旧版本）
-- ============================================================
...
-- ============================================================
-- 7. 清理任务：定期清理过期的 rate_limit_window 记录
-- ============================================================
